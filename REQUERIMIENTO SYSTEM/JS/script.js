function processRequerimiento() {
    ...
    if (userConfirmed) {
        ...
        doc.text(`Requerimiento de Material de Empaque - Correlativo: ${correlativo}`, 105, 25, { align: "center" });
        
        // Datos Generales
        rows.push(["Día", dia]);
        rows.push(["Hora", hora]);
        rows.push(["Supervisor", supervisor]);
        rows.push(["Linea", linea]);
        ...
        if (cajas.length > 0) {
            ...
            cajas.forEach((caja, index) => {
                ...
                rows.push([`Caja PAQ ${index + 1}`, cajasPaq]);
                rows.push([`Caja Descripción ${index + 1}`, cajasDesc]);
            });
        }
        ...
        if (bobinas.length > 0) {
            ...
            bobinas.forEach((bobina, index) => {
                ...
                rows.push([`Bobina UNI ${index + 1}`, bobinaUni]);
                rows.push([`Bobina Descripción ${index + 1}`, bobinaDesc]);
                rows.push([`Bobina Peso/Kilos ${index + 1}`, bobinaPeso]);
            });
        }
        ...
        if (otros.length > 0) {
            ...
            otros.forEach((otro, index) => {
                ...
                rows.push([`Otro CANT ${index + 1}`, otrosCant]);
                rows.push([`Otro Descripción ${index + 1}`, otrosDesc]);
            });
        }
        ...
        // Redirigir a Gmail con el enlace
